...
conn = mysql.connector.connect(user='root', password='Lonelyrider16ce133', host='127.0.0.1', database="phonepe")
...
if SELECT == "Home":
    ...
    col1.image(Image.open("C:/Users/muges/Downloads/Phonepe_image2.png"),width = 300)
    with col1:
    ...
    with col2:
        st.video("C:/Users/muges/Downloads/upi.mp4")
...
# Top Charts - USERS          
    if Type == "Users":
        ...
        with col2:
            st.markdown("### :violet[District]")
            cursor.execute(f"select district, sum(Registered_user) as Total_Users, sum(App_opens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} group by district order by Total_Users desc limit 10")
        ...
        with col3:
            st.markdown("### :violet[State]")
            cursor.execute(f"select state, sum(Registered_user) as Total_Users, sum(App_opens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} group by state order by Total_Users desc limit 10")
...
# EXPLORE DATA - TRANSACTIONS
    if Type == "Transactions":
        ...
        with col1:
            ...
            df2 = pd.read_csv(r"C:/Users/muges/Downloads/Statenames.csv")
            df1.State = df2
...
# EXPLORE DATA - USERS      
    if Type == "Users":
        ...
        cursor.execute(f"select state, sum(Registered_user) as Total_Users, sum(App_opens) as Total_Appopens from map_user where year = {Year} and quarter = {Quarter} group by state order by state")
...
if SELECT == "About":
    col1,col2 = st.columns(2)
    with col1:
        st.video("C:/Users/muges/Downloads/pulse-video.mp4")
    with col2:
        st.image(Image.open("C:/Users/muges/Downloads/Phonepe_image2.png"),width = 500)
        st.write("---")
    ...
    with col1:
        ...
        st.image(Image.open("C:/Users/muges/Downloads/about_phonepe.jpg"),width = 400)
        with open("C:/Users/muges/Downloads/about_phonepe1.png","rb") as f:
            data = f.read()
        st.download_button("DOWNLOAD REPORT",data,file_name="annual report.pdf")
    with col2:
        st.image(Image.open("C:/Users/muges/Downloads/about_phonepe1.png"),width = 800)
...
if SELECT == "Contact":
    Name = (f'{"Name :"}  {"MUGESHKUMAR"}')
    mail = (f'{"Mail :"}  {"mugeshk403@gmail.com"}')
    description = "An Aspiring DATA-ANALYST..!"
    social_media = {
        "GITHUB": "https://github.com/Mugeshkumar133",
        "LINKEDIN": "www.linkedin.com/in/mugeshkumar33"}
    
    col1, col2, col3 = st.columns(3)
    col3.image(Image.open("C:/Users/muges/Downloads/mugesh.jpg"), width=300)
    st.subheader(Name)
    st.subheader(mail)
    with col2:
